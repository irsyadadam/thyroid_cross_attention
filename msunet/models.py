...
class MSU_Net_Full(nn.Module):
    def __init__(self, img_ch=1, output_ch=1):
        super(MSU_Net_Full, self).__init__()
...
class MSU_Net(nn.Module):
    def __init__(self, img_ch=1, output_ch=1):
        ...
        self.Att1 = Attention_block(F_g=32,F_l=32,F_int=1)
    ...
    def forward(self, x):
        ...
        d4 = self.Up4(x4)
        x3 = self.Att3(g=d4,x=x3) # remove later
        d4 = torch.cat((x3, d4), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        x2 = self.Att2(g=d3,x=x2) # remove later
        d3 = torch.cat((x2, d3), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        x1 = self.Att1(g=d2,x=x1) # remove later
        d2 = torch.cat((x1, d2), dim=1)
        d2 = self.Up_conv2(d2)
