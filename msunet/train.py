...
def train_msunet(FOLD_NUM: Optional[int]):
    r"""
    ...
    "\n-----------Model Load-----------"
    #load model etc
    MODEL_WEIGHTS = Path("MSUNET_uclaweights.pth")
    DEVICE = torch.device(f'cuda:{device_ids[0]}' if torch.cuda.is_available() else 'cpu')
    model = nn.DataParallel(MSU_Net(), device_ids = device_ids).to(DEVICE)
    ...
    scheduler = CosineAnnealingLR(optimizer, T_max=100, eta_min=0.1*LR, verbose=True)
